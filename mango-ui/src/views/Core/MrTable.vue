    ...
    <el-table :data="data.content" :highlight-current-row="highlightCurrentRow" @selection-change="selectionChange" 
      ...
      <el-table-column :label="$t('action.operation')" width="360" fixed="right" v-if="showOperation" header-align="center" align="center">
        <template slot-scope="scope">
          <kt-button icon="fa fa-eye" label="查看" :perms="permsLook" :size="size"  @click="handleSee(scope.$index, scope.row)" />
          <kt-button icon="fa fa-plus" label="新增" :perms="permsAdd" :size="size"  @click="handleaddInfo(scope.$index, scope.row)" />
...
export default {
  ...
  methods: {
    ...
    // 编辑
     ...
     // 新增详细信息
		handleaddInfo: function (index, row) {
      this.$emit('handleaddInfo', {index:index, row:row})
		},
    // 删除
